...
# changeLog
# 编译时添加 PAM 模块
# 修复了 在 CentOS 6.8下 回滚操作后，有几率重启SSH失败
# 
clear
build_date="20220817"
build_version="v2.1.4"
...
function fun_check_root(){
    if [[ "$(id -u)" != "0" ]]; then
    echo -e "$(message_error_tag) 当前用户不是 root，无法执行，退出脚本"
    exit 1
    else
...
function fun_install_dependencies_packages(){
 if [[ $INSTALL_FORCE = 'false' ]];then
            ...
            if [[ $REQUEST == "Y" ]]; then
                ...
                echo -e "$(message_info_tag) 运行以下命令安装依赖包: yum install -y $PACKAGES_LIST wget"
                echo "" && exit 1
            fi
